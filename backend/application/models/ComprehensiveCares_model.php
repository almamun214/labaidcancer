 ...
 * @author Sagor
 */
class ComprehensiveCares_model extends CI_Model {

    function __construct() {
        parent::__construct();
    }

    protected $tbl_comprehensive_care = "comprehensive_care";
  
    function add_comprehensive_care($filename) {
        $data = array(
            'cc_name' => $this->input->post('cc_name'),
            'image' => $filename
        );
        $this->db->insert($this->tbl_comprehensive_care, $data);
    ...
    function comprehensive_care_list() {
        return $this->db->get($this->tbl_comprehensive_care)->result();
    }

    function single_comprehensive_care($id) {
        return $this->db->get_where($this->tbl_comprehensive_care, array('cc_id' => $id))->row_array();
    }

    function delete_comprehensive_care($id) {
        $this->db->trans_begin();
        
        $this->db->where(array('cc_id'=>$id));
        $this->db->delete($this->tbl_comprehensive_care);
...
//        return $this->db->query($sql)->result(); 
    ...
    function update_comprehensive_care($filename, $id) {
        $data = array(
            'cc_name' => $this->input->post('cc_name'),
            'image' => $filename
        );
        $this->db->where(array('cc_id' => $id));
        $this->db->update($this->tbl_comprehensive_care, $data);
        if ($this->db->affected_rows() > 0) {
            return true;
        } else {
            return false;
