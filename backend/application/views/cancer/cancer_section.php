    ...
    <div class="card-body">
        ...
        <form method="" id="myform">
            <div class="form-group">
                <label for="title">Section Title <span class="required">*</span></label>
                <input type="text" class="form-control" id="title" name="title"  placeholder="Section Name">
                <small  class="warning"></small>   
            </div>

            <div class="form-group">
                <label for="content">Section Details </label>
    ...
    <script>
        ...
        function addSection() {
            ...
            if (sectionname == null || sectionname == "") {
                Altext += "Section Name Required \n";
            }
            if (Cancerdetails == null || Cancerdetails == "") {
                Altext += "Section Details Required";
            }
            ...
            if (Altext == "") {
                $.ajax({
                    ...
                    url: "<?php echo base_url('cancer/addSection'); ?>",
                    dataType: 'json',
        ...
        function displayData() {
            ...
            var content = "<table class='table table-striped'><thead><tr><th>Section Name</th><th>Action</th></tr></thead><tbody>";
            var cnid = $("#cnid").val();
            $.ajax({
                type: 'POST',
                data: {cnid: cnid},
                url: "<?php echo base_url('cancer/displayData'); ?>",
                dataType: 'json',
                success: function (response) {
                    var count = 1;
                    $.each(response, function (index, value) {

                        content += "<tr><td>" + value.cd_section_title + "</td><td><button class='btn btn-danger' onclick='removeData(" + value.cd_id + ")' title='Remove'>X</button> </td></tr>";
                    });
        ...
        function removeData(id) {
            ...
            $.ajax({
                type: 'POST',
                data: {id: id},
                url: "<?php echo base_url('cancer/removeDataCancer'); ?>",
                dataType: 'json',
                success: function (response) {
                    displayData();
                }
